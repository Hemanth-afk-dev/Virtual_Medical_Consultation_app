...
const Login = () => {
  ...
  const demoCredentials = [
    ...
    {
      role: 'Doctor',
      email: 'ananya.rao@mediconnect.com',
    ...
    {
      role: 'Pharmacist',
      email: 'priyanka.das@mediconnect.com',
      password: 'Priyanka@2026',
    },
  ];
  ...
  useEffect(() => {
    if (location.state?.role && ROLES.includes(location.state.role)) {
  ...
  const fillDemoCredentials = (credential) => {
    ...
    setError('');
    setSuccess(`Demo credentials loaded for ${credential.role}`);
  };
              ...
              <div className="demo-list">
                {demoCredentials.map((credential) => (
                  <button
                    key={credential.role}
                    type="button"
                    className="demo-item demo-item-btn"
